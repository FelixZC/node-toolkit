...
export default declare(
  (
     ...
     */
    const replaceMemberExpressionObject = (
      path: NodePath<t.MemberExpression>,
    ) => {
      let ref = generator(path.node.property).code;
      /** 是否为匹配属性 */

      const replaceObj = (options.replaceObjectList || []).find(
      ...
      if (replaceObj) {
        ...
        if (
          ...
          replaceObj.upLevelObjectName ===
            generator(path.node.object.property).code
        ) {
     ...
     * @param path 表示MemberExpression节点的路径。
     */
    const replaceMemberExpressionProperty = (
      path: NodePath<t.MemberExpression>,
    ) => {
      let ref = generator(path.node.property).code;
      const replaceObj = (options.replacePropertyList || []).find(
        (item) => item.property === ref,
      );
      if (replaceObj) {
        // 如果满足特定条件，则替换节点的属性。
        ...
        Program: {
          exit(path) {
            ...
            for (const ref of refList) {
              const isExist = valueLocalreplaceObjectList.some(
                (item) => item.localName === ref,
              );
