 ...
 */
export const findObjectPropertyWithKey = (
  ...
  const result = target.properties.find((item) => {
    if (t.isObjectProperty(item) || t.isObjectMethod(item)) {
      const taget =
        (item.key as t.Identifier).name || (item.key as t.StringLiteral).value;
      return taget === key;
    }
    return false;
  });
...
export const filterSameObject = (
  ...
  for (const item of elements) {
    ...
    if (target) {
      const key =
        (target.key as t.Identifier).name ||
        (target.key as t.StringLiteral).value;
...
export const getFunctionName = (path: NodePath<t.Function>) => {
  ...
  if (t.isArrowFunctionExpression(path.node)) {
    const definedNodePath = path.findParent((path) =>
      t.isVariableDeclarator(path.node),
    );
    definedNodePath &&
      (result = (
        (definedNodePath.node as t.VariableDeclarator).id as t.Identifier
      ).name);
  } else if (
 ...
 */
export const createObjectTemplateNode = (
  input: Record<string, any> | string,
) => {
...
export const replaceExpressionProperty = (
  ...
  // 如果找到匹配项，则为其添加新属性
  if (matchObjIndex > -1) {
    elements[matchObjIndex] = addObjectNewProperty(
      elements[matchObjIndex],
      newProperty,
    );
  }
  return elements;
...
export const resetIndexObjectProperty = (elements: t.ObjectExpression[]) => {
  ...
  for (const element of elements) {
    ...
    if (indexProperty && indexProperty.value) {
      ...
      const newIndexProperty = t.objectProperty(
        t.identifier("index"),
