module.exports = (fileInfo, api) => {
  ...
  const localName = importDeclaration.find(j.Identifier).get(0).node.name // current order of arguments
  ...
  return root
    ...
    .replaceWith((nodePath) => {
      const { node } = nodePath // use a builder to create the ObjectExpression
